...
describe('lib/sitemap.mjs — buildSitemapProposal: visit-mismatch блокує ПАРУ', () => {
  ...
  it('термінація: заблокована пара більше не пропонується, цикл зупиняється', async () => {
    // Обидва кандидати на `product` — сітки, тож обидва відхиляються пробом.
    // Доводиться саме те, що заблокована пара не пропонується вдруге: жодна
    // пара (url, тип) не відвідується двічі, і цикл зупиняється сам.
    // 🔴 Не «інакше був би вічний цикл»: термінацію тримають бюджет візитів і
    // скінченність множини пар — див. шапку `lib/sitemap.mjs`.
    const { visit, calls } = fakeVisitWithProbe({
      'https://a.com/': HOME,
