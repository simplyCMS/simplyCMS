...
const SERVER_PAYLOAD = [
  ...
  // дві, бо форма в `dist` задається профілем збірки, а не декларацією: на
  // чинному tsup усі шість субшляхів емітяться теками
  // (`dist/admin-server/impl/…`, `dist/storefront/loaders/…`), а гілка
  // `\.js` тримає пласку форму (`dist/db.js`) — щоб зміна профілю збірки
  // мовчки не знімала гейт із субшляху.
