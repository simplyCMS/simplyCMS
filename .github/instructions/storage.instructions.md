...
- Рядок `media` і файл пишуться **однією транзакцією актора** —
  `writeMedia(db, …)`; видалення — `eraseMedia(db, ref)`, і порядок там
  зворотний: СПЕРШУ `DELETE` рядка, ПОТІМ обʼєкт, обидва до COMMIT. Причина —
  необоротна дія стоїть якомога пізніше: за зворотним порядком відмова БД
  після `driver.delete` знищила б файл безповоротно (рішення Е2-7).
