...
### 🔴 Борг: теги релізів не ведуться

Стан на 2026-08-08, перевірено по коду й по remote:

- `release.mjs:58` тег **перевіряє** (`tagExists`), але `git tag` на створення
  немає ніде в `scripts/release/`. Тобто жоден реліз тега не залишає;
- `git.mjs:49` читає **локальні** теги (`git tag --list`), а не remote. У
  свіжому клоні список порожній, тож гард пропустив би навіть повторний випуск
  уже виданої версії;
- у remote лежить рівно один тег — `v0.1.0`, і він **вводить в оману**: указує
  на коміт `dd822d6` часів scope `@simplysoftua`, тобто до перейменування і до
  першої публікації на npmjs. Реліз `0.1.0` у реєстрі зроблено з іншого коміту.

Чому це не діра, а втрачена зручність: від повторного випуску реально захищає
`compareVersions` проти версії в манифестах (`release.mjs:51`) — вона в git,
тож працює й у свіжому клоні, — плюс `isAlreadyPublished` усередині
`pnpm publish -r`.

Що треба вирішити при закритті боргу (це дизайн, а не однорядковий фікс):
тег ставить людина локально чи CI **після** успішної публікації (логічніше
друге — тоді тег означає «опубліковано», а не «підготовлено»); і чи гард
питає remote (`git ls-remote --tags`) замість локальних тегів. Історію релізів
доти веде [`CHANGELOG.md`](../../CHANGELOG.md).
