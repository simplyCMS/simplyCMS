...
- 🔴 **дія власника — блокер мержу гілки, що вводить цей пакет у `main`,
  а не абстрактне «перед першим релізом».** Тригер публікації — будь-який push
  у `main`, бампу версії для цього не треба (`isAlreadyPublished` пропускає
  лише те, що вже в реєстрі — див. «Що робить CI після мержу» нижче); отже
  сам факт мержу вже запускає спробу опублікувати `create-simplycms-store`.
  Чинний secret `NPM_TOKEN` — Granular Access Token, обмежений scope
  `@simplycms` (`publish-packages.yml:15`). Unscoped `create-simplycms-store`
  він **не покриває**, а granular-токен не можна заздалегідь розширити на
  пакет, якого ще немає в реєстрі npm. **До мержу** власник має або
  (а) видати новий токен «Read and write» на **всі** пакети акаунта (з Bypass
  2FA) і замінити secret, або (б) опублікувати першу версію
  `create-simplycms-store` вручну зі своєї машини (`npm publish` з робочого
  каталогу пакета) і лише потім звузити токен назад до scope `@simplycms` для
  решти 21. Без цього мерж або червонить job (токен не покриває), або тихо
  публікує пакет поза релізним рішенням (токен розширений завчасно).
