# SimplyCMS Core → Headless Commerce Engine: дизайн перерозподілу пакетів та reuse у MetaHub

> Статус: дизайн-чернетка для обговорення. Код не змінюється.
...
> 2. MetaHub HUB — домен **Продукти** (тільки товарні механізми, scoped по `hub_id`, інтеграція з інвойсами);
> 3. MetaHub Marketplace — публічна вітрина (storefront + SSR + SEO);
