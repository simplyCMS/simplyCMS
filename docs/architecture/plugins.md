...
1. **dependency-lint** (eslint-зона `plugins/**` +
   ...
   що мовчки відвалилось, теж дає зелений лінт.
   🔴 **Борг після К0 (не регресія — стартова зона була байт-ідентична
   доконсолідаційній, звірено з `d81bac1`):** до К0 звернення плагіна до
   інших внутрішніх шарів ядра вимагало вписати новий `@simplycms/*` у
   власний манифест — тертєвий крок, видимий у ревʼю; тепер усе ядро —
   субшляхи однієї залежності `simplycms`, тож тертя зникло. Зона з тих пір
   розширена вручну (B9, контур v2) під кожен новий обхідний шлях до БД.
