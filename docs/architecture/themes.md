...
2. **Ідемпотентність**: тека `themes/<key>` існує І конфіг уже має
   `import('@themes/<key>/index')` → успіх-нічого-робити.
3. **Колізія**: тека існує, а конфіг на неї не вказує → помилка ДО будь-яких
   дій (підмінити чужу теку мовчки не можна).
4. `pnpm add <pkg>` — пакет ставиться ТИМЧАСОВО.
5. Валідація `node_modules/<pkg>/src/index.ts` (конвенція форми §3.1) —
...
7. Копія `src/*` → `themes/<key>/` (+ README.md/LICENSE з кореня пакета,
   якщо є).
8. Запис у конфіг `'<key>': () => import('@themes/<key>/index')`.
9. `pnpm remove <pkg>` — пакет знімається, тема лишається локальною текою.
