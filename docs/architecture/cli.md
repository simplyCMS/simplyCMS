...
CLI — інструмент **магазину**: він виконується в теці магазину,
...
🔴 Виняток — **авторські команди `create` (theme/plugin) і
`theme:conformance`**: вони працюють ще й у корені **монорепо ядра**, бо і
скаффолд теми/плагіна, і гейт її views потрібні автору саме там.
Резолвер кореня один (`findRoot` у `context.mjs`), різниця — один прапорець:
`findStoreRoot` (doctor/add/update/db:diff) визнає лише магазинний маркер,
`findScaffoldRoot` (`create`, `theme:conformance`) — ще й монорепо-маркер. Розширювати виняток
на решту команд не можна: у монорепо `doctor` дав би хибні error-и по
`routes.ts` і `tailwind.config.ts` (там теки монтуються шляхами, а імпорти
йдуть аліасами — обидва коректні за побудовою), а `update --write` копіює
`packages/cli/host/` у корінь, тобто в напрямку, ЗВОРОТНОМУ до
...
🔴 Єдина команда, яка приймає ще й корінь **монорепо ядра** (`findScaffoldRoot`,
маркер `pnpm-workspace.yaml` + `simplycms.config.ts` в одній теці) — див. §1.
Створює й дописує вона тільки НОВЕ, тож у монорепо це безпечно; решта команд
там і далі гучно відмовляють «корінь магазину не знайдено».

### 3.6 `simplycms theme:conformance <name>`

Conformance-гейт контракту тем **v3** (`views`) — КАНОНІЧНИЙ канал запуску,
доступний магазину БЕЗ тестового рушія. `<name>` — ключ теми в
`simplycms.config.ts` (через `--name` при установці він може не збігатися ні з
...
🔴 У звіті друкуються сторінки, які kit **справді прогнав** (його повернене
значення), а не заявлені в `theme.views`: заявка без кейса в kit-і не
перевіряється, і друк заявленого показував би прогін, якого не було.

🔴 **jsdom вантажиться динамічно і в шаблон магазину НЕ їде** (рішення власника
2026-08-18): важкий тулчейн ставиться on-demand із гучною інструкцією — та
сама доктрина, що Playwright у скілі редизайну. Немає пакета — команда друкує
рівно `pnpm add -D jsdom` і виходить кодом 1, без мовчазних обходів. React,
react-dom і vite у магазині вже є: `vite` тут — завантажувач модулів (тема
живе в TS/TSX), аліаси беруться з `compilerOptions.paths` магазинного
`tsconfig.json`, бо раннер vite примусово працює з `configFile: false`.

Другий канал — файл `conformance.test.ts`, який `create theme` кладе в теку
теми з коробки: він для магазину/CI, де vitest уже є. Магазин без рушія
тестів цей файл просто ігнорує — гейт лишається доступним CLI-командою.

## 4. Канонічні артефакти: звідки CLI знає «як має бути»

Канони закріплені parity-тестами; два з трьох — **закомічені копії,
синхронізовані `pnpm template:sync`** (той самий механізм, що й шаблон
скаффолдера):

| Джерело правди | Копія | Споживач |
|---|---|---|
| `SYNCED_FILES` (11 host-файлів глю, корінь монорепо) | `packages/cli/host/**` | `update`, `doctor` §6 |
