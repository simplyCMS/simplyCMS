...
Доносить нові core-міграції в магазин. До появи цієї команди магазин **не мав
шляху** отримати міграції нової версії ядра: tarball `@simplycms/schema` їх не
віз, а копія в магазині фіксувалася в момент скаффолда.

Порівнюються `supabase/migrations/` магазину і
...
- 🔴 спільне ім'я з різним вмістом → **error, нічого не пишеться** (міграції
...
## 9. Поза скоупом v1 (свідомо)

`create plugin`/`plugin:dev` (авторський цикл), монтаж `adminRoutes` плагінів,
drizzle-композиція плагінних схем у `db:diff` — Фаза 3 (Plugin SDK); copy-in
тем — Фаза 4; semver-перевірка `engines.simplycms` — реліз-потяг v1.0;
AST-редагування конфігу — усвідомлене спрощення v1 (якірна вставка з чесним
падінням), переглянути з ускладненням конфігу.
