...
- [x] **Step 1:** `SKILL.md` — рівно перелічені місця Р11 (+Р7 механізм `--out`); `.claude/commands/редизайн-за-референсом.md` — звірити формулювання. — Фаза 1 переписана на «дискавері → обовʼязковий діалог із таблицею „знайшов ↔ unresolved“ → інспекція кожної підтвердженої сторінки в `<slug>/<pageType>/`» із прямою забороною мовчазного продовження; фаза 2 — мультивхід із обовʼязковим `--out` і `sources`; доданий розділ «Межі v1» (reverse-анімації, сторінки за логіном, десктопний семпл, `@import`-шрифти, «структура не переноситься») і рядок у «Чесній деградації»; усі приклади — канонічним шляхом `node .claude/skills/redesign-from-reference/scripts/…`. У команді-обгортці оновлено `description` і додано другий чекпойнт.
- [x] **Step 2:** `docs/guides/redesign-from-reference.md` — кроки discover→діалог→inspect×N→map-tokens×N, час скролу, десктопний семпл; `docs/tasks/platform-roadmap.md` — відмітка інкремента в розділі треку. — Гайд перенумеровано (кроки 1-4), додано розділ дискавері, поведінки скролу/семплу/радіус-sanity, мультивхід мапінгу, 4 рядки діагностики, шпаргалка й межі v1. У роадмап додано підрозділ «Інкремент Б.1» з пʼятьма закритими пунктами, чесним DoD і новим боргом `lab()`/`oklab()`.
- [x] **Step 3:** `pnpm template:sync` + повний канонічний порядок гейтів + `pnpm pilot:pack`. — Синк оновив копію `SKILL.md` у шаблоні; гейти зелені (`lint` 0 errors / 13 warnings — норма, `test` 527/527, `test:packaging` 10/10), пілот A/C/D/CLI/TOOL — PASS, Gate B/E — очікуваний SKIP без БД.
- [ ] **Step 4:** Відмітки в задачі (DoD §3.1–3.3; §3.4 лайв-тест — наступний крок) і PR #32. — Відмітки в задачі й плані проставлено (§3.1 закрито чесно: скрол/радіуси так, «кольорів >5» — ні, з причиною й боргом). PR #32 **не створювався**: гілку свідомо не пушено — це дія власника.
