...
- [x] **Step 3:** 🔴 **закомітити ВСЕ** (`git status` чистий) — `scripts/release.mjs:62` має `assertCleanTree()` ДО бампу; потім `pnpm release 0.4.0` (гарди + бамп 5 + гейти + коміт). ✅ коміт `7b38c94`, усі 8 гейтів зелені, 5 пакетів на 0.4.0.
- [x] **Step 4:** push гілки; PR у `main` — НЕ мержити без власника. ✅ чернетковий PR #42; у шапці явно виділено, що мерж = публікація і зайняття unscoped-імені.
