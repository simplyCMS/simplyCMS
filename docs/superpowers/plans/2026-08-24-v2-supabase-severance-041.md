...
- [X] **Step 4:** 🔴 закомітити ВСЕ (`git status` чистий — `release.mjs` має
      `assertCleanTree()` до бампу), далі `pnpm release 0.4.1`.
      ✅ Виконано: коміт `0bf805e`, усі 9 гейтів релізу зелені, версія 5 пакетів
      `0.4.0` → `0.4.1`.
- [X] **Step 5:** push, PR у `main`. 🔴 Мерж = публікація 0.4.1 — рішення власника.
      ✅ Виконано: [PR #45](https://github.com/simplyCMS/simplyCMS/pull/45).
      🔴 **Мерж НЕ зроблено — він за власником.**

---

## Підсумок контуру (для наступної сесії)

**Стан:** усі 8 задач плану закриті, рев'ю пройдено (6 лінз + адверсаріальна
перевірка, два кола фіксів), версія в коді `0.4.1`, PR #45 відкритий, у реєстрі
npm поки `0.4.0`.

**Що знайшло рев'ю і що виправлено в цьому ж контурі** — три дефекти, доведені
живими прогонами: Gate B пілота ходив у БД голим `select` під роллю без грантів;
вісім місць доків друкували накат канону як `psql "$DATABASE_URL"`, хоча
`app_runtime` не може ні `create schema`, ні `alter role`, ні `create table`;
тест генерації `BETTER_AUTH_SECRET` пропускав зашитий спільний секрет. Деталі —
`CHANGELOG.md`, розділ «Виправлено за результатами рев'ю контуру».

**Наступний трек — К3 (адмінка):** 53 файли / 215 звернень `.from()` з браузера,
серверних функцій нуль. Потребує власної спеки перед стартом (рішення власника
2026-08-24). Після К3 зникає останній споживач `supabase-js`.

**Незакриті борги контуру** — 0.4.1-1…11 у [роадмапі](../../tasks/platform-roadmap.md),
зокрема три дефекти безпеки (owner-invite DoS, відсутній `profiles` у власника,
витік `admin_comment`), сліпота RLS-матриці на крос-акторний запис і відсутність
команди накату схеми.
