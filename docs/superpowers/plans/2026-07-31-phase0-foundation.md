...
- [ ] **Step 4 (нуль-diff gate):** ПЕРЕД експериментом — checkpoint незакоміченого baseline у свіжу теку: `BASE=$(mktemp -d) && cp -r packages/simplycms/schema/drizzle "$BASE/"` (не сталий шлях — повторний запуск не зіпсує checkpoint). Далі `pnpm drizzle-kit generate --config …` → згенерований SQL **порожній/відсутній**; якщо diff є — правити `schema.ts` до нуля (типи/дефолти). Відкат експерименту: `rm -rf packages/simplycms/schema/drizzle && cp -r "$BASE/drizzle" packages/simplycms/schema/` (git checkout непридатний — baseline ще не закомічено) → `git status` показує лише навмисні нові файли схеми.
