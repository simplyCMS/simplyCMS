...
2. **Контролі прогнані вручну — негативні І позитивні:**
   ...
   - Task 7 Step 6 — пряма перевірка тотожності інстансу: `useQueryClient()`
     у дереві компонентів === `Route.useRouteContext().queryClient` дає
     `true` з проброшеним `customQueryClient` і `false` без нього.
     🔴 Попереднє формулювання («прибраний `customQueryClient` дає повторні
     запити після гідрації») знято фінальним рев'ю Е1а як артефакт виміру:
     дозамір із очищеним трекером мережевих запитів дає рівно 8
     `_serverFn`-запитів за ОБОХ конфігурацій (попередні «16» були
     неочищеним лічильником між сесіями браузера), а мосту SSR→браузер для
     кешу React Query в проєкті немає (пакет не встановлений, нуль
     `dehydrate`/`hydrate`) — заявленого механізму подвоєння запитів не
     існує.
