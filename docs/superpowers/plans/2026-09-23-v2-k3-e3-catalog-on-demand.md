...
| Е3-20 | 🔴 *(архітектор; live:smoke Task 13, 2026-09-24)* **Доменні помилки serverFn переживають RPC-межу через ОДИН `domainErrorAdapter`** (`simplycms/runtime`, client-safe; перелік імен і полів — `simplycms/contracts`), зареєстрований у host `src/start.ts` (`serializationAdapters`, канон host-файлів + `template:sync`); адаптер іде ПЕРЕД дефолтними плагінами Start. Обхід `normalizeThrown` прибрано (TSDB-5 — хибна модель межі) | Живий прогін: «дубль slug → тост i18n» падав — Start серіалізує будь-який `Error` лише як `message` (`ShallowErrorPlugin`), тож `AdminConflictError`/`AuthzError` приходили голим `Error(message)`. Хибним виявився і пункт (3) спеки К3-13 (виправлено). Юніт-тести конфліктів були зелені, бо моделювали помилку в тому ж процесі, не перетинаючи серіалізацію — урок у `test-contours.md` §11.1 |
