...
describe('writeMedia / eraseMedia проти живої БД (Е2, Task 3)', () => {
  ...
  it('тимчасові файли після всіх прогонів не лишились', async () => {
    for (const shard of await readdir(root)) {
      ...
      expect(files.filter((n) => n.endsWith('.tmp'))).toEqual([]);
    }
  });
});
```
 ...
 *
 * 🔴 Порядок — СПЕРШУ обʼєкт, ПОТІМ рядок (рішення Е2-7). Зворотний
 * порядок при обриві лишив би файл, якого ніхто не обліковує; цей — рядок
 * без файлу, тобто облік ПЕРЕоцінює обсяг. Переоцінка безпечна (тариф не
 * занижений), а повтор проходу її виправляє, бо `driver.delete` на
 * відсутньому обʼєкті — успіх.
 *
...
🔴 Якщо поруч лишився дублюючий заголовок `{t('profile.settings.avatar')}` (рядок ≈221) — прибери його: компонент тепер підписує інпут сам, і дві копії підпису зробили б `label` неоднозначним.
