...
| Е2-7 | 🔴 **ПЕРЕГЛЯНУТО 2026-09-13.** Видалення обʼєкта — **всередині транзакції рядка, ПІСЛЯ `DELETE` і ДО `COMMIT`**; `unlink` з `ENOENT` = успіх | Початкове «спершу обʼєкт, потім рядок» (§4-К4) виявилось неточним: за ним будь-яка відмова БД після `driver.delete` знищувала файл безповоротно. Справжній інваріант — не «обʼєкт перед рядком», а «обʼєкт усередині транзакції рядка»: тоді помилка сховища відкочує рядок, помилка БД не торкається обʼєкта, а fail-open неможливий за побудовою. Найгірший залишковий стан той самий. Розбір — `simplycms-de` під час виконання, рішення архітектора, спеку амендовано тим же комітом |
...
- 🔴 **звіт виконавця про зелений гейт — НЕ доказ зеленого гейта.** Виміряно
  на хвилі B: два виконавці відрапортували повний ланцюг зеленим, один навіть
  перелічив його поіменно включно з `format:check`, — а вісім файлів було
  закомічено без `prettier --write`. Достатня умова тут одна: **гейти
  перевірено ПІСЛЯ хвилі тим, хто її приймає**, а не переказано з рапорту;
- 🔴 **крок «має бути ЗЕЛЕНИМ одразу» — не формальність, а прихована
  перевірка ПРИПУЩЕНЬ ПЛАНУ.** Там, де план каже прогнати щойно доданий
  гейт і очікує зелене без жодної правки, він насправді перевіряє не код, а
  те, чи автор правильно описав реальність. Червоне на такому кроці —
  сигнал, що помилився ПЛАН, і виправляти треба реєстр, список чи
  формулювання очікування, а **не підганяти гейт під зелене**. Це не
  теорія: `tests/media-columns-coverage.test.ts` (Task 1, Step 6) на
  першому ж прогоні знайшов дві розбіжності з реєстром, який автор плану
  склав вручну — `services.image_url` бракувало, `users.image` був хибним
  позитивом. Такі кроки в плані ще є: Step 3 Task 5 (i18n-гейти) і Step 9
  Task 6 (харнес вітрини «PASS без правок»);
 ...
 */
export function sniffImageMime(bytes: Uint8Array): MediaMime | null {
  // 🔴 Поріг — 8, довжина найдовшої сигнатури БЕЗ зсуву (PNG). Сигнатури зі
  // зсувом (WebP на 8, AVIF на 4+8) на короткому вході не матчаться самі:
  // `at()` порівнює з `undefined`. Поріг 12 відкидав би валідний 10-байтний
  // PNG — спіймано фікстурою власного тесту.
...
describe('writeMedia / eraseMedia проти живої БД (Е2, Task 3)', () => {
  ...
  it('app_user: відмова на рядку, ФАЙЛ і рядок лишаються цілі', async () => {
    ...
    // 🔴 Текст Postgres лежить у `.cause`, не в `message`: Drizzle перезагортає
    // драйверну помилку в `Failed query: …`. Патерн уже задокументований у
    // `with-actor.test.ts:152-160` цього ж харнеса — беремо його, а не
    // винаходимо власний.
...
export async function writeMedia(
  ...
  // 🔴 `id: id`, а НЕ шортхенд `id`: гейт `explicit-ids` розпізнає лише явну
  // форму (`insert-scan.ts:69`, `ID_FIELD = /(^|[\s{,])id:\s*\S/`), і так
  // написані всі наявні вставки ядра. Шортхенд пройшов би `tsc`, але крок
  // «`explicit-ids` зеленим одразу» впав би — і це саме той випадок, коли
  // помилився ПЛАН, а не код.
...
🔴 **Крок існує тому, що коментаря в харнесі НЕДОСТАТНЬО**: його побачить лише
той, хто відкриє саме цей файл, а ти працюєш у `core/lib` і `profile-ui` й
туди можеш не зайти жодного разу. Без явного кроку кейс не зʼявиться ніколи —
і найцінніший тест послідовності заміни (єдиний, що доводить порядок «новий →
профіль → старий» проти відкоту) лишиться ненаписаним.

Текст кейсу — у Task 3 Step 1, блок «відмова на видаленні СТАРОГО». Перенести
його дослівно, зняти коментар-борг із шапки файлу, прогнати:
...
5a. Харнес `media-record` має **девʼять** кейсів: восьмий (послідовність заміни зі зламаним видаленням старого) додано в Task 5, коментар-борг із шапки знято.
