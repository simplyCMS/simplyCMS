...
- [x] **Step 1:** клієнт+пул+withActor ≤150 рядків/файл, server-only гард
      (за зразком `server-client.ts`).
      🔴 Файлів ЧОТИРИ, а не три: `actor.ts` (тип актора, валідація ролі/uuid,
      ЧИСТА побудова преамбули) винесено з `with-actor.ts` — інакше форму SQL
      і валідацію аргументів не можна довести юнітом БЕЗ Postgres, а вони
      мусять доводитись саме так. `pg` переїхав у `dependencies` ядра
      (магазин драйвер не імпортує — це деталь реалізації), `@types/pg` — у
      devDependencies пакета.
- [x] **Step 2:** інтеграційні тести через харнес у `test:schema`-контурі.
      🔴 Побічний ефект `@types/pg`: `tsc` почав ВИВОДИТИ типи `apply.mjs` для
      `.test.ts` (checkJs вимкнено, але інференс працює) — `client.query()`
      резолвився в overload `R extends any[]`, і пʼять наявних гейтів упали з
      TS2352/TS2345. Полагоджено явним `@returns {Promise<any[]>}` у
      `queryRows`/`queryInTransaction`.
- [x] **Step 3:** лінт-зона + негативний контроль; повні гейти + packaging
      (новий export у tarball-parity).
      🔴 Зона розкладена на ТРИ конфіг-блоки (`eslint.db-client-zone.mjs`):
      flat config замінює опції правила цілком, тож тір-зони й межа довіри
      плагінів доливають групу до своїх патернів явно — інакше заборона
      мовчки зникала б рівно там, де ціна помилки найвища.
...
- [x] **Step 1:** `pnpm install` (нова залежність) → далі frozen-ланцюг.
      🔴 `better-auth` — точний пін `1.7.1` у `dependencies` ядра (B3),
      `@types/pg` уже стояли з Task 6.
- [x] **Step 2:** інстанс + хуки + юніти на memory-адаптері.
      🔴 Три знахідки рантайму, кожна ламала контур мовчки або гучно:
      (а) `drizzleAdapter` хоче ОДИН довгоживучий db-обʼєкт, а `withActor`
      живе рівно транзакцію — знято `drizzle-orm/pg-proxy` (`auth/
      drizzle-proxy.ts`): кожен запит адаптера стає власною короткою
      транзакцією під `app_admin`, тож лінт-зона `db/client` лишається
      недоторканою. Транзакції proxy не тримає, і це безпечно: у 1.7.1 усі
      `db.transaction` адаптера стоять під `provider === 'mysql'`;
      (б) `advanced.database.generateId` мусить бути `'uuid'`, а НЕ `false`:
      drizzle-pg оголошує `supportsUUIDs` (id генерує БД), а memory-адаптер
      ні — з `false` юніт-контур лишався б без id;
      (в) `memoryAdapter({})` падає «Model user not found» — сховище треба
      заводити ключами моделей в ОДНИНІ (`usePlural` — справа drizzle-
      адаптера).
      🔴 Побічно закрито латентний розрив: специфікатор `simplycms/schema`
      усередині монорепо не резолвився взагалі (аліас веде в ТЕКУ, а
      `index.ts` там не було) — поки схему ніхто не імпортував, це було
      невидимо. Додано барель `src/schema/index.ts`; `exports`-мапа й
      entry-точки tsup не змінені, тож `dist/` лишається дзеркалом exports.
- [x] **Step 3:** інтеграція: signUp на харнесі → сесія → `withActor` з
      claims сесії → RLS віддає лише свої рядки (замикання «сесія→RLS»).
      Файл — `test-harness/pg/__tests__/auth-integration.test.ts` (5 кейсів):
      провізія профілю категорією `retail` із сіду, інваріант
      `first_user_no_auto_admin` асертиться як «рядків `admin` у таблиці
      НУЛЬ», ланцюг id-із-сесії → `withActor` → RLS із контрольною парою
      (анонім бачить 0, чужий id — лише чуже), invite наживо (випуск →
      гасіння → роль `admin`) та його ідемпотентність.
- [x] **Step 4:** authz-хелпери + тести матриці; повні гейти.
      `requireOperation` повертає SCOPE, а не `void` — інакше різниця
      `own`/`any` була б невидимою на місці виклику. Реалізації scope немає
      (дизайн-насіння §8), є вимір і матриця.
...
- [x] **Step 1:** порт логіки + вичерпні юніти (оператори умов, пріоритети,
...
- [x] **Step 1:** типи + компіляційні тести (expectTypeOf на 2-3 таблицях).
      Приземлено: `src/schema/types.ts` (~60 типів рядків), субшлях
      `simplycms/schema/types`, `__tests__/types.test.ts` (5 асертів на
      `users`/`orders`/`profiles`). 🔴 Чутливість гейта перевірена живцем:
      навмисна підміна `User['id']` на `number` дала `TS2344`, після відкату
      зелено — тобто асерти справді ловлять регресію, а не зелені намарно.
- [x] **Step 2:** доки + роадмап; повні гейти + `pnpm pilot:pack`.
      Оновлено: `CLAUDE.md` (Quick Reference `test:schema`, Project Structure
      `src/db`/`src/auth`/`src/schema/types.ts`/`test-harness/`, ключі env
      контуру V2, зникнення `supabase/migrations/`),
      `docs/architecture/test-contours.md` (новий §10 — що доводить контур,
      три негативні контролі й ЧОГО він не доводить), обидва README
      (заморозка дуальних типів до К1′б), роадмап (К1′а виконана + борги
      К1а-1…К1а-7).

---

## 🔴 Що лишилось НЕдоведеним (чесний борг, не галочка «за наміром»)

Приземлення 2026-08-23, гілка `claude/roadmap-next-phase-gzayf7`. Усі дев'ять
задач виконані й закомічені, гейти зелені — але три речі **фізично недосяжні**
в CI-оточенні й не можуть вважатися доведеними:

1. **Browser-e2e на оновлення сесійної cookie після refresh** (DoD К1′,
   спека §4). Playwright і Chromium є, але спеки потребують живого стеку, а
   на К1′а вітрина ще на `supabase-js` — застосунок під новим auth не
   підіймається. Доводиться лише на оточенні власника, після К1′б.
2. **SMTP-доставка invite-листа** (борг №1 роадмапу). Рендер листа —
   юніт-тестовний і покритий; сама доставка через Mailpit/SMTP — ні.
3. **Живий прогін проти реального стека** (`pnpm pilot`/`pilot:e2e`) —
   потребує Docker або живої БД; обидва відсутні.

Плюс борг процесу: **верифікаційне рев'ю К1′а не проведено** в сесії
реалізації (вичерпано ліміт). Перед мержем гілки — обов'язково
`/перевір-роботу-агента-кодування` з цим файлом як аргументом.

Повний перелік — розділ «Борги треку V2-К1а» (К1а-1…К1а-7) у
[роадмапі](../../tasks/platform-roadmap.md).
