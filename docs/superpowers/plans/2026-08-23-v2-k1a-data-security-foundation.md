...
- [ ] **Step 1:** інвентаризація поточної `schema.ts` (1026 рядків) —
      зафіксувати список таблиць/енамів, що переносяться як є.
- [ ] **Step 2:** `auth.ts` (BA-таблиці, uuid; конфіг генерації id — на
...
- [ ] **Step 3:** FK-retarget шести таблиць (якорі в довідці) + дзеркала в
      `relations.ts`; видалити `auth-users.ts`.
- [ ] **Step 4:** RLS-ядро: переписати політики user-scoped таблиць у
      ...
      переносити.
- [ ] **Step 5:** B7-поля + індекси; `pnpm lint`/`typecheck`/`test` зелені
...
**Interfaces:**
- Produces: канон міграцій = `0001_init` + `0002_seed`; `simplycms db:diff`
  рахує від baseline; жоден дефолтний гейт не посилається на видалені файли.

- [ ] **Step 1:** згенерувати `0001_init` (drizzle-kit проти снапшотів);
      руками НЕ дописувати (виняток — блок ролей/грантів із Task 4, якщо
      порядок вимагатиме одного файлу: тоді явний маркер-коментар).
- [ ] **Step 2:** накат `0001_init`+`0002_seed` на харнес — зелено, без
      шима (у baseline немає `auth.users`/`storage.*` — це і є доказ B13).
- [ ] **Step 3:** видалення старої теки + перекомутація всіх споживачів
      (список у Files; повний радіус звірити orient-ом на місці).
- [ ] **Step 4:** `pnpm pilot:pack` зелений; `pilot:e2e`/`db:migrate` —
...
- [ ] **Step 1:** фікстурні актори + мінімальні дані (не сід магазину —
      власні рядки тесту).
- [ ] **Step 2:** матриця + негативні контролі; `pnpm test:schema` зелений.
- [ ] **Step 3:** перевірити, що зміна предиката будь-якої політики в
      `schema.ts` без регенерації baseline червонить парність (жива
      перевірка гейта — один навмисний диф, відкат).
