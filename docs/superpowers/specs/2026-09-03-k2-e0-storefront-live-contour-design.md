...
> §5, §6, §7, §8, Додаток Б і Додаток В; додано Додаток Г. 🔴 **Ред. 1.4
> (2026-09-13): серверна квота чекауту** — рішення архітектора (сесія
> simplycms-e1, `architect-decisions.md`, розділ M), ухвалене ПІД ЧАС
> виконання Task 11: показане покупцю й записане замовлення ділять ОДНУ
> server-only функцію `prepareCheckout`, тож розбіжність показ/запис
> (знижка з реальної кількості, дрейф прайсу, порогова доставка) закрита
> ЗА ПОБУДОВОЮ. У спеці змінено Е0-4.
>
...
🔴 **Ред. 1.4 (2026-09-13): серверна квота — спільний `prepare` із
оформленням.** Причина — три джерела розбіжності показ/запис, знайдені
ПІСЛЯ ред. 1.3: клієнтський знімок знижок кошика проти серверної знижки з
реальною кількістю й сумою; дрейф прайсу в `localStorage`; ПОРОГОВА
доставка (`free_from`/`min_order_amount`) — поріг можна перетнути на
клієнті й не перетнути на сервері, аж до хибної відмови ПІСЛЯ показаної
ціни. Рішення (архітектор, `architect-decisions.md`, розділ M): одна
server-only функція `prepareCheckout` (`storefront/loaders/prepare-checkout.ts`)
— довідники, ціни й тариф БЕЗ запису — ділять і `placeOrderFor`, і нова
`quoteCheckoutFor` (`storefront/loaders/quote-checkout.ts`, serverFn
`quoteCheckout` у `storefront-routes/server/checkout-quote.ts`); контракти
T0 — `QuotedItem`/`CheckoutQuote`/`QuoteCheckoutResult`
(`contracts/objects/order.ts`). Клієнт: `Checkout.tsx` перезапитує квоту з
дебаунсом ~300 мс на зміну кошика/доставки/юзера (гейт на `hydrated`,
Task 12); `CheckoutOrderSummary`/`CheckoutQuoteDetails` показують суми
КВОТИ, не клієнтського кошика, СКЕЛЕТ до першої відповіді; `canSubmit`
вимагає свіжої вдалої квоти. Гейт «показане = записане» —
`checkout-flow.test.ts` (M-10a/b/c): `quote.total === orders.total` на
тому самому вході. Названі межі: `/cart` лишається знімком; вікно між
квотою й кліком закриває `order-success` (показує ЗАПИСАНІ числа);
`expectedTotal`/`total_changed` — беклог К2, адитивний.
...
| 🔴 Ред. 1.3: стан магазину змінився МІЖ замовленням і скасуванням — тумблер `decrease_on_order` вимкнули, точку деактивували, — і повернення розійшлося зі списанням | повернення йде за ТИМ САМИМ правилом точки й тією ж умовою тумблера, що й списання, тож розбіжність можлива лише при зміні конфігурації між двома подіями; для замовлення з `orders.pickup_point_id` (типовий випадок і весь демо-контур) точка зафіксована в рядку й не дрейфує зовсім — крім видалення самої точки: FK `orders_pickup_point_id_fkey` — `ON DELETE set null` (`0001_init.sql:651`), тож поле обнуляється і повернення йде за загальним правилом (системна → перша активна). 🔴 **Ред. 1.5 (2026-09-13, після фінального рев'ю гілки):** мітигація вище була написана для випадку з `orders.pickup_point_id` і для замовлення БЕЗ самовивозу НЕТОЧНА — там поле порожнє, тож `resolveStockPoint` кликався окремо на списанні й на поверненні, і деактивація точки між двома подіями відправляла залишок у ІНШУ точку (механізм відтворено прямим SQL на харнесі). Закрито БЕЗ DDL: `reserveOrderStock` пише резолвлену точку в `orders.shipping_data.stock_point_id` (наявна jsonb-колонка — саме той шлях, який цей же абзац називає для видаленої точки), `releaseOrderStock` читає її ПЕРШОЮ, а резолв лишається відкатом для замовлень, оформлених до правки. Точний облік «скільки з якої точки взято» (кілька точок на одне замовлення) вимагав би окремої сутності з власним DDL, грантами й RLS — свідомо відкинуто (Додаток Б, блок ред. 1.3), бо етап лишається без зміни схеми |
