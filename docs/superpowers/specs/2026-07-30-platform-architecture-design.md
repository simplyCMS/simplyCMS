...
> 🔴 **Ревізія 2026-08-17 (рішення власника): D3/D4 еволюціонували в
> D3′/D4′.** Дані, роутинг і SEO канонічних сторінок — як і раніше
> невідʼємна частина ядра; нове — тема МОЖЕ перевизначати **view-шар** пʼяти
> сторінок вітрини (Home, Catalog, CatalogSection, ProductDetail, Cart)
> через типізовані view-model-и під жорстким conformance-гейтом. D2 чинне
> без змін. Повна спека ревізії —
> [`2026-08-17-theme-contract-v3-views-design.md`](2026-08-17-theme-contract-v3-views-design.md);
> згадки D3/D4 нижче по тексту читати з урахуванням цієї ревізії.

> 🔴 **Ревізія 2026-08-19 (рішення власника): D7 еволюціонував у D7′ —
> бекенд-контракт v2.** Supabase перестає бути обовʼязковим фундаментом
> продукту: цільовий контракт — **сервер-first дані (браузер не звертається
> до БД; PostgREST/GoTrue/supabase-js зникають) + Better Auth + storage-порт
> + чистий Postgres**; Supabase стає одним із підтримуваних провайдерів
> (його Postgres + S3-ендпоінт Storage). Клієнтів і реальних магазинів
> немає — реструктуризація без зворотної сумісності (розширення D5).
> D8 доповнено ухваленими спеками маркетплейсу і хмари. Джерела правди
> ревізії: [`2026-08-19-backend-contract-v2-design.md`](2026-08-19-backend-contract-v2-design.md),
...
> згадки Supabase-шару нижче по тексту (зокрема §2bis п.5-6, D6-частину
> «Supabase CLI») читати з урахуванням цієї ревізії.

> 🔴 **Ревізія 2026-08-20 (рішення власника): пакетна топологія
> ревізується.** Спека
> [`2026-08-20-package-consolidation-design.md`](2026-08-20-package-consolidation-design.md)
> запускає окремий механічний трек К0 (виконується ПЕРЕД треком V2) —
> консолідація пакетів **26 → 5**: unscoped `simplycms` (флагман) +
> сателіти `@simplycms/cli`, `create-simplycms-store`,
> `@simplycms/theme-solarstore`, `@simplycms/plugin-faq`. У коді ще НЕ
> реалізовано. Багатопакетну розкладку, описану нижче в цьому документі
> (§3, §4), читати з урахуванням цієї ревізії.
...
> 🔴 **Ревізія 2026-08-20:** розкладка нижче ревізована спекою
> [`2026-08-20-package-consolidation-design.md`](2026-08-20-package-consolidation-design.md)
> — консолідація 26 → 5 (трек К0 перед V2). Таблицю НЕ переписано (лишається
> історичним фактом); чинний напрям пакетної топології — у спеці К0.
...
- **Один монорепо платформи.** Окремий core-репозиторій (subtree-дзеркало
  `packages/simplycms`) і git-subtree-синхронізацію **виведено з експлуатації**:
  ...
  `src/`). Колишнє subtree-дзеркало видалено (2026-07-31); уся історія — в монорепо.
...
### 5.1. Амендмент за фактом Фази 4 (2026-08-14)

Фактичний механізм — **не** `@source`-директиви в `globals.css`, а
`content`-глоби в `tailwind.config.ts` шаблону скаффолдера (явний список
шляхів `node_modules/@simplycms/*/dist/**/*.js`, `node_modules/@simplycms/*/routes/**`,
з Фази 4 — `node_modules/simplycms-theme-*/dist/**/*.js` і
`node_modules/@*/simplycms-theme-*/dist/**/*.js` для сторонніх тем). CLI
нічого не генерує: глоби статичні, покривають конвенцію неймінгу пакетів
(Р1 плану Фази 4), а не перелік конкретних встановлених пакетів — новий
пакет під тією самою конвенцією підхоплюється без правок конфігу. Деталі —
[`docs/architecture/themes.md`](../../architecture/themes.md) §3.3, доведено
...
### 6.1. Амендмент за фактом Фази 4 (2026-08-14)

`ThemeManifest` фактично несе четверте поле — `displayName: string` (окремо
від `name`, який лишається технічним ідентифікатором/ключем реєстрації):
`{ name, displayName, version, engines: { simplycms } }`
(`packages/theme-system/src/types.ts`). Причина: `name` — ключ конфігу й
рядка БД (не може довільно змінюватись без розсинхрону резолву), а
`displayName` — те, що бачить адмін у списку тем; сплутування цих ролей в
одному полі зробило б UI або технічним, або нестабільним. Гард —
`theme-manifest-parity` (непорожній `displayName` для референс-пакетів).

### 6.2. Амендмент за фактом v2.2 (2026-08-15)

Контракт розширено типографікою і власним каналом шрифтів теми.
`ThemeTokenValues` несе два не-кольорові ключі — `'font-sans'` і
`'font-heading'` (значення — повний CSS `font-family` stack рядком, як
`"'Manrope', system-ui, sans-serif"`; прецедент не-кольорового токена —
`radius`), `ThemeModule` — опційне
`fonts?: ReadonlyArray<{ stylesheet: string }>` з абсолютними `https:`-URL
зовнішніх stylesheet-ів, які каркаси `StorefrontShell`/`ProtectedShell`
рендерять `<link>`-ами поруч із `ThemeTokens`. `@font-face` і роздача
файлів шрифтів темою лишаються поза межею свідомо: npm-тема не має каналу
статики. Тим самим кроком із контракту зникли brand-змінні — `--brand-*` і
`colors.brand` видалено, три utility-класи `.gradient-brand*` зберегли
імена, але фарбуються `--primary` активної теми (доти перемикання теми
градієнти воронки не чіпало — діра контракту, а не дизайн-рішення).
Нумерація «v2.2» — версія КОНТРАКТУ теми (після v2 «токени замість
сторінок» і v2.1 «каталог `messages`»), а не фаза роадмапу: жодної фази
платформи цей трек не відкриває й не закриває. Докази факту — юніти
`packages/theme-system/src/__tests__/` (`applyTokens` рендерить нові ключі й
відсіює ключ поза `TOKEN_KEYS`, `safeFontStylesheets` відкидає не-`https:` і
значення з лапками, `validateThemeModule` звіряє форму `fonts`) і
grep-доказ нуля споживачів `--brand-*`/`colors.brand` поза трьома
переписаними класами, знятий по всьому репо ПЕРЕД видаленням змінних.
Рішення D2–D4 не змінюються: сторінок тема як не несла, так і не несе.
