...
- Теми: `themes/default` — без власних `fonts` (успадковує Inter-fallback);
  ...
  явний `fonts` зі stylesheet Inter (нуль візуальних змін; механізм
  доводиться компонентним тестом на реальному модулі — ревʼю плану
  показало, що пілот fonts-контур бачити не може: Gate D читає лише
  зібраний CSS, а сід активує `default`) — див. Р8/Р9 плану.
...
1. ~~Чи давати solarstore власний `fonts` — і якщо так, чи розширювати
