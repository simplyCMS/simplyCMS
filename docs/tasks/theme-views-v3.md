...
- [ ] A: view-model-и пʼяти сторінок у `@simplycms/objects` із секційною
      структурою; `views?` у контракті; `validateThemeModule` мʼяко
      перевіряє форму; всі наявні теми працюють без правок.
- [ ] B: slot-компоненти з маркерами `data-simplycms-requisite`; логіка
      купівлі/кошика перенесена БЕЗ зміни поведінки (наявні тести зелені).
- [ ] C: пʼять сторінок розкладені на container + canonical view; view —
...
- [ ] D: conformance-kit рендерить заявлені views на фікстурних vm і
      червоніє на зламаній синтетичній темі (негативний контроль);
      шаблон `create theme` везе conformance-тест.
- [ ] E: ланцюг settings доведений (БД+default-и, інвалідація SSR-кешу
      при зміні, conformance з default-ами) — з тестами.
- [ ] F: доки оновлені; `pnpm template:sync`; parity зелений.
- [ ] Гейти зелені канонічним порядком + `build:packages`/`test:packaging`
      (зачеплені exports пакетів objects/theme-system/storefront-routes).
- [ ] 🔴 `pnpm pilot:pack` зелений: спліт сторінок міняє модульний граф
