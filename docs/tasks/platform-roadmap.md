...
**Фази 0 і 1 завершені. Ядро опубліковане на npmjs — `@simplycms/*@0.2.2`, усі 21
пакет, плюс unscoped `create-simplycms-store` тієї ж версії (разом 22).**

> Версія `0.2.2` стає дійсною в реєстрі **в момент мержу** цього релізу в `main`
> (тригер workflow `publish-packages`). Історія змін —
> [`CHANGELOG.md`](../../CHANGELOG.md); перевірка, що доїхали всі 22 пакети, —
> `pnpm verify:published 0.2.2`.
...
4. **i18n-міграція — закрито ЧАСТКОВО (2026-08-09).** Мігровано два пакети:
   1495 кириличних рядкових вузлів у 76 файлах (а не 954 — стільки бачив лінт,
   тобто 64 % роботи; решту — toast, Zod, тернарники, мапи ярликів — не бачив).
   Каталог: 961 ключ у 32 модулях, `uk` і `en` дзеркальні файл у файл.
   `warn→error` розширено з переліку 3 файлів на обидва пакети цілком.
   🔴 Завершеність доводить **не** лінт, а `tests/i18n-coverage.test.ts` —
   AST-скан із порожнім `PENDING_FILES`.

   🔴 **Чого міграція НЕ покрила** (аудит 2026-08-09 тим самим AST-сканером,
   але по всіх теках UI, а не по двох `SCANNED_ROOTS`): воронка покупки
   лишилась хардкодженою українською — `checkout-ui` 123 рядки,
   `profile-ui` 82, `catalog-ui` 38, `reviews-ui` 27, `cart-ui` 13,
   `core/hooks/{useStock,useProductReviews}` 10; плюс теми
   (`default` 36, `solarstore` 43). Разом ~283 рядки UI + ~79 у темах.
   Тому теза плану «магазин англійською збирається зміною одного рядка
   конфіга» **не виконується**: при `locale: 'en-US'` сторінка англійська, а
   шапка й підвал теми — українські (доведено скріншотом
   `/catalog`, смок 2026-08-09). Наступний етап — «i18n: воронка покупки».
   🔴 Передумова етапу: у цих п'яти пакетах 128 рядків містять **латинську `i`
   всередині кириличних слів** («Мiсто», «Прiзвище», «Iнший отримувач»).
   Чистити їх окремим проходом не треба — міграція переписує ці рядки
   однаково; але скопіювати їх у каталог **не можна**, інакше сміття
   зафіксується в `MessageKey`-значеннях назавжди.

   ✅ **Візуальна перевірка виконана 2026-08-09** (борг закрито): 27 роутів
   × 2 viewport (1280×900 і 390×844) × 2 локалі, вимір переповнення в
   реальному Chromium (`scrollWidth > clientWidth`, вихід за межі
   контейнера, горизонтальний скрол документа). Результат: **єдина**
   EN-специфічна різниця — 2 px на «Percentage (%)» у мобільному селекті
   `/admin/discounts/new`. Ризики, названі у звіті PR #27, **не
   підтвердились**: «Price: low to high» = 114 px у тригері 178 px;
   `Days of the week` із сімома чекбоксами вміщається; заголовки таблиць
   `Actions`/`First name` не обрізаються. 0 помилок консолі в обох локалях.
...
7. **Теги релізів не ведуться** (виявлено при релізі `0.2.2`). `pnpm release`
   тег перевіряє, але ніколи не створює, а перевіряє **локальні** теги — тож у
   свіжому клоні гард сліпий. Єдиний тег у remote (`v0.1.0`) указує на коміт
   часів scope `@simplysoftua`, тобто позначає не той реліз. Це втрачена
   зручність, а не діра: від повторного випуску захищає порівняння версії в
   манифестах і `isAlreadyPublished` у pnpm. Історію релізів веде
   [`CHANGELOG.md`](../../CHANGELOG.md); дизайн фіксу (хто ставить тег —
   людина чи CI після публікації) описано в
