...
К0-7. **Стейл канону, НЕ породжений К0** (знайдено при синхронізації доків;
   свідомо не правлено — це чужа задача, і латати її мимохідь усередині К0
   означало б розмити межу зміни):
   - `AGENTS.md` і `docs/marketplace/README.md` кажуть «контракт тем **v2**»,
     тоді як чинний канон — v3 (трек A, 2026-08-18);
   - `AGENTS.md` і `.github/instructions/architecture-core.instructions.md`
     обіцяють «~960 warn на ще не мігровані кириличні рядки» — i18n-міграцію
     завершено, чинна норма `pnpm lint` = 0 errors / 13 warnings.
   Обидва місця активно вводять в оману нового агента: він звірить свій
   прогін із обіцяним числом і вирішить, що зламав репо.
