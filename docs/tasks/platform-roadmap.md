...
К0-8. **`<title>`/`<meta description>` роутів вітрини не перекладені**
   ...
   `<title>`** — найпомітнішу SEO-поверхню. Закриття = рішення власника про
   спосіб доставки локалі в ядро (глобальний акцесор конфігу / `head()` з
   контексту роутера / вихід у V2-контракт), а не правка рядків.
