...
агентом (повний пайплайн скіла на живому магазині) — наступний крок, поза цим
DoD.

#### Борги, свідомо винесені за межі етапу Б

- 🔴 **Живий прогін по зовнішньому референс-сайту** (і фази 4-5 скіла:
  компоненти за спеками + side-by-side QA) — потребує зовнішньої мережі,
  браузерного MCP і судження про схожість; дія власника. Зараз контур доводять
  лише фікстурний HTML (`page.setContent`, нуль мережі) і юніти мапінгу.
...
- 🔴 **CSS Color 4 в семплері (`lab()`/`oklab()`) — відкритий дефект,
  знайдений живою пробою інкремента Б.1.** Chromium серіалізує computed-колір
  у тому просторі, у якому його оголошено: на Tailwind v4 з oklch-токенами це
  1224 з 1289 непрозорих значень сторінки. Наш `toHex` парсить лише
  `rgb()/rgba()` і решту мовчки дропає — саме тому жива проба лишилась при
  пʼятьох колірних кластерах навіть ПІСЛЯ фікса скролу (доведено окремим
  probe-скриптом). Скоуп — `lib/color.mjs`, поза межами рішень Р1/Р1b/Р2, тому
  свідомо не чіпалось у цьому інкременті.
