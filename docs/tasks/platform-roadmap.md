...
## 📍 Поточний стан (оновлено 2026-08-23)
...
3. **Трек V2 — Бекенд-контракт v2** (спека 2026-08-19 З АМЕНДМЕНТОМ
   ...
   - [ ] К1′ Фундамент — модель даних і безпеки v2 (спека §4 п.1, блоки 0–6):
     ...
     - [ ] **К1′б — точка неповернення** (план пишеться після К1′а):
           admin-guard на BA-сесіях, перемикання auth-роутів клієнта,
           знос GoTrue/`simplycms/supabase`-фабрик; тягне мінімальний
           зріз К2 (вітрина мусить жити); довга гілка до зеленого
