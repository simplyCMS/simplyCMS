...
3. **Трек V2 — Бекенд-контракт v2** (спека 2026-08-19 З АМЕНДМЕНТОМ
   ...
   - [ ] К1′ Фундамент — модель даних і безпеки v2 (спека §4 п.1, блоки 0–6):
     ...
     - [ ] **К1′а — адитивна половина**
           ([план](../superpowers/plans/2026-08-23-v2-k1a-data-security-foundation.md)):
           PG-харнес + гейт `test:schema` (CI без Docker), схема v2
           (BA-таблиці, FK-retarget з `auth.users`, B7-поля комерції,
           таблиця медіа-метаданих К4, індекси), baseline+сід чистого
           магазину з видаленням 33 старих міграцій (B13), ролі+гранти
           як код + гейт парності привілеїв, поведінкова матриця RLS,
           `simplycms/db` (withActor), `simplycms/auth` (Better Auth
           server-side, databaseHooks, authz-хелпери), category-rules →
           TS-домен, типи з Drizzle (частина B12)
