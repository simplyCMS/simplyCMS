...
> (theme views). Детальний імплементаційний план кожного треку пишеться
> перед стартом (superpowers writing-plans) і посилається звідси.
> Переписано 2026-08-19: виконані Фази 0–4 і трек редизайну стиснуто до
> архіву (§«Виконано»), повна історія — у git цього файлу і в планах фаз.

---

## 📍 Поточний стан (оновлено 2026-08-19)

**Фази 0–4 завершені. Ядро опубліковане на npmjs — 26 пакетів однією
версією `0.3.0`** (25 `@simplycms/*` + unscoped `create-simplycms-store`).
Магазин створюється `pnpm create simplycms-store` і збирається зі справжніх
npm-пакетів; CLI (`simplycms doctor/add/create/update/db:diff`), плагіни
(SDK + референси), теми як пакети (npm і copy-in), контракт
маркетплейс-індексу — працюють. Механізм «редизайн за референсом»
обкатаний двома живими прогонами; його результат — нинішня
`themes/default`.
...
| Теми | контракт v2.2 (tokens+components+messages+fonts); npm (`@simplycms/theme-solarstore`) або copy-in; `bootstrapThemes` |
| Редизайн за референсом | скіл `redesign-from-reference` (дискавері → інспекція → мапінг → side-by-side) |
...
1. **Бекенд-контракт v2** (ревізія D7): сервер-first дані (браузер не
   ...
   контрактів портів першим стороннім пакетом) і ПЕРЕД треком A тем.
...
2. **Трек V2 — Бекенд-контракт v2** (спека 2026-08-19; імплементаційний
   ...
   - [ ] К2 Вітрина: лоадери через репозиторії (злиття двох шляхів
         data-access), full-page кеш + CDN-заголовки + інвалідація,
         waterfall/N+1-фікси, JSON-LD на всіх канонічних сторінках +
         поля повернень/доставки в схему, AI-robots + llms.txt,
         зображення (розміри/трансформації), **фасетна навігація** за
   ...
   - [ ] К5 Екосистема: порти плагінів v2 (бекенд-нейтральні), порти для
         тем (знімає Р10), оновлення референсів і conformance
   - [ ] К6 Тулчейн: типи з Drizzle (зникає дуальна система), пілот/e2e/
         сід під Postgres-стек, CLI/doctor, шаблон скаффолдера, доки
3. **Трек A — контракт тем v3 «theme views»** (спека 2026-08-17) — ПІСЛЯ
   V2. 🔴 Ревізія порядку V5 (2026-08-19): було «ревʼю → C → A → B»,
   стало «C (виконано) → **V2** → A → B» — обидва треки чіпають сторінки
   вітрини, view-шар має лягати на нові лоадери, а не переписуватись
   двічі. Трек B (секційна модель + drag-and-drop) — після обкатки A.
...
9. **`bootstrapPlugins`/`bootstrapThemes` — клієнтський `useEffect` без
   серверного контуру; бізнес-емітери hooks (`order.created`) не емляться;
   `plugin:purge` і облік `plugins.migrations_applied` відсутні** —
   переглядаються в V2-К5 (частина знімається новим контрактом портів).
10. **Живий SSR-доказ fonts-контуру** (Р9 етапу А редизайну) — потребує
...
(фази 0–6 повністю, гейти 593/593). Хвіст — ре-валідація (черга №1);
борги — №7, №10.
