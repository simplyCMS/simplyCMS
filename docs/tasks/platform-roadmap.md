...
2026-08-15/16** (тема `deo`; підсумки і прогалини — інкремент Б.2 нижче).
...
- ~~🔴 **Живий прогін по зовнішньому референс-сайту** (і фази 4-5 скіла)~~ —
  **закрито 2026-08-15/16 лайв-тестом**: агент пройшов повний пайплайн
  ...
  `themes/deo` (гейти 531/531). Виявлені прогалини процесу — скоуп
  інкремента Б.2 нижче.
...
- **Межі інспекції v1:** `fontStylesheets` бачить лише `link[rel=stylesheet]`
  ...
  sweep — робота агента, не скрипта.
...
#### Інкремент Б.2 (трек C): шліфування механізму після лайв-тесту — заплановано 2026-08-17

Джерело — повний лайв-тест скіла (фази 0–5 проти реального референсу,
тема `themes/deo`, гейти 531/531). Хотфікси знахідок (CLI-guard через
realpath, canvas-семплер) уже в гілці PR #32; прогалини процесу — скоуп
цього інкремента. Артефакти інспекцій (`docs/design-references/`) з
...
Задача і план — перед стартом (порядок: після ревʼю-циклу і мержу PR #32).
...
- [ ] **Motion-капчер**: transitions/keyframes у семплері, reveal-дифи при
      скролі, детект JS-анімацій (GSAP/Framer/Lottie) з чесною деградацією;
      секція «Motion» у шаблоні спек; автоматизований hover-sweep
      (Playwright `hover()` — закриває беклог «interaction sweep»).
- [ ] **Demo-датасет діагностичного стенда**: дамп клауд-БД АльтерЕнерджі →
      окремий rich-сід (описи, фото, властивості, ціни) — без нього
      канонічна картка товару на стенді порожня і QA сліпе. Сід пілота НЕ
      чіпається (контракт гейтів); стенд до клауд-БД напряму НЕ
      підключається (`bootstrapThemes` писав би в живий магазин).
- [ ] Нова **опційна фаза «Шліфування»** у скілі: посторінковий прохід із
      драбиною рішень: токен → setting теми → (з треком A) view теми →
      кастомна сторінка `src/routes/my/` → свідомо прийняти канон.
- [ ] Беклог лайв-тесту: `findStoreRoot` відмовляє `simplycms create theme`
      в монорепо; шрифт-евристика (голосування по span-мітках обрало
      Geist Mono замість Geist).
- [ ] **Фінальна валідація = повторний прогін механізму на референсі deo**:
      шліфування теми до професійного рівня.
- [ ] **`deo` → npm-пакет `@simplycms/theme-deo`** (зразок — solarstore) +
      **дефолтна активна тема шаблону `create-simplycms-store`**
      (`themes/default` лишається еталоном fallback-токенів). 🔴 Мерж цієї
      фази в `main` опублікує НОВИЙ пакет у реєстрі — за чинним правилом це
      релізне рішення в момент мержу.
