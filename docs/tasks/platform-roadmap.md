...
- **Реліз/бамп версій** — трек міняв `exports` і вміст tarball-ів
  ...
  `@simplycms/theme-solarstore` (а трек A — ще й `@simplycms/objects`,
  `@simplycms/cli`); коли це поїде в реєстр — рішення власника.
...
- [x] Нова **опційна фаза «Шліфування»** (фаза 6) у скілі: посторінковий
      ...
      фіксується в артефакті. Щабель «view теми» після треку A став
      реальним (контракт тем v3, 2026-08-18) — драбина в скілі оновлена.
...
#### Інкремент Б.3: дискаверер v2 + чесність вимірювальних каналів — виконано 2026-08-18 (крім ре-валідації проти живого референсу)
...
гілка `claude/redesign-increment-b3`.

- [x] **Словники платформ + type-aware тайбрейк** (`classify-terms.mjs`,
      `classify.mjs`): симетрія однини/множини, bare-форми конфліктних
      сегментів (`/product`, `/item`, `/tovar`), синоніми Woo/Wix/
      Squarespace/VTEX/Rozetka, ru-набір якорів; при рівному балі для типу
      `product` виграє ГЛИБШИЙ pathname (у картки є slug), для решти —
      коротший, як було. Сімʼя `c<id>` свідомо не введена (на Prom.ua це
      профіль продавця), `.html` — теж (нуль розрізнювальної сили).
- [x] **Структурний fan-out** — новий чистий `lib/classify-structure.mjs`
      (+ `lib/classify-pairs.mjs`, бо канон 150 рядків): префікс із ≥3
      прямими дітьми-кандидатами дістає evidence
      `{ structural: 'prefix-fanout', count, source: 'structure' }` як
      `listing`, бездітний лист під ним — `fanout-leaf` як `product`.
      Корінь `/` виключено. Межа сигналу названа в доках: `/blog` із 12
      постами структурно не відрізняється від `/shop` із 12 товарами —
      вирок лишається за діалогом фази 1.2.
- [x] **Контент-верифікація візиту** — `lib/visit-probe.mjs`: JSON-LD
      `@type` (обхід довільної вкладеності, `@graph`/`itemListElement`)
      плюс лічильник карток; груба невідповідність дає нову причину
      **`visit-mismatch`** тим самим шляхом перепідбору, що `visit-failed`.
      JSON-LD — додатковий сигнал, не вимога.
- [x] **`sitemap-proposal.json` `schemaVersion: 2`**: голе число `linksSeen`
      замінено масивом `links: [{ pathname, anchors }]` — увесь вхід
      класифікатора (V-3; діагностика більше не потребує власного зонда).
- [x] **Чесність reveal-каналу** (V-5): корінь — діти `<main>`, а без нього
      `body > section` ПЛЮС діти обгорток верхнього рівня (стеля 60 вузлів);
      нові поля `revealSampled`/`revealRoot`; `jsDrivenSuspected` став
      **триставним** — `'unknown'` при нульовій вибірці, тобто «не виміряно»
      більше не невідрізненне від «перевірено, підозри немає».
- [x] **Структурований `unmapped`** (V-6+V-7) — новий `lib/unmapped.mjs`:
      записи `{ hex, role, count, area, contrastOnBackground,
      contrastOnCard, belowAA }`, дедуп за парою (hex, роль), стабільне
      сортування; `belowAA` — за найгіршою з двох поверхонь, лише для ролі
      `text`. `tokens-proposal.json` → `schemaVersion: 2` (форма зламана,
      бамп чесний). Тихий провал контрасту немапованого кольору закрито.
- [x] **Скіл/гайд/шаблон**: фаза 1 скіла — нові сигнали і `visit-mismatch`
      людськими словами для діалогу 1.2; таблиця `motion` — обсяг вибірки й
      три стани прапорця; фаза 2 — структурований `unmapped`; фаза 4 —
      що писати у спеку при стані «невідомо»; **нова фаза 5.4** — чек-лист
      браузерної перевірки фільтрів (V-8: чиста сторінка на кожен стан,
      очікування з БД, обовʼязковий контрольний непорожній випадок);
      нотатка V-1 — політика артефактів (магазин комітить, монорепо ядра
      тримає `docs/design-references/` у `.gitignore`). `pnpm template:sync`
      прогнано, parity зелений.
- [x] **Раунд рев'ю після приземлення** (2026-08-18). Знайдено два справжні
      дефекти. Перший — **підтверджений хибний позитив `visit-mismatch`**:
      картки рахувались як «лінки, глибші за поточну сторінку», а на Shopify
      (`/collections/all` → `/products/<slug>`) і WooCommerce індекс і картки
      лежать на однаковій глибині, тож лічильник обнулявся рівно там, де
      потрібен, і коректний каталог відхилявся. Лікування: розмір найбільшої
      сімʼї однопрефіксних лінків із зображенням; `CollectionPage`/`ItemList`
      знімає правило `listing`; проб знімається ПІСЛЯ прокрутки; помилка
      самого проба більше не перетворює 2xx-сторінку на `visit-failed`; а
      відхилення блокує ПАРУ (тип, pathname), не сторінку — «під карткою
      виявилась сітка» це доказ на користь `listing`, і глобальне вирізання
      знищувало б доказ разом із помилкою. Другий — **сліпа зона CI на
      корені reveal**: логіка вибору кореня жила лише в browser-gated тесті,
      який у CI скіпається. Закрито CI-безпечним jsdom-дублем
      (`tests/design-import-reveal-dom.test.ts`); туди ж дописано покриття
      `revealSampled`, `rgbToHex` (`padStart`), причини ОСТАННЬОГО кандидата
      в `unresolved`. Плюс прибрано два джерела дрейфу: поріг «розрідженої»
      сторінки виведено з `FANOUT_MIN_CHILDREN` замість другого літерала,
      агрегація якорів зведена в одну `groupLinksByPathname`. Версії:
      `inspection.json` → 3 (нові reveal-поля), сумісність — `[1, 2, 3]`.
- [x] **Друге коло рев'ю** (2026-08-18) — фікс сімʼї карток вніс
      ДЗЕРКАЛЬНИЙ хибний позитив, і його теж закрито. Сімʼя рахувала
      оточення самої сторінки: топ-рівневі іконкові лінки ділять префікс
      `/` (шість — уже «сітка»), а на картці `/products/<slug>` блок
      «схожі товари» ділить із нею батьківський префікс `/products/`. На
      магазині без `Product`-розмітки тип `product` через це не резолвився
      взагалі й з'їдав бюджет візитів. Обидві сімʼї — `/` і батьківський
      префікс поточної сторінки — тепер не рахуються ніколи; цільові
      розкладки збережено (картки там під іншим префіксом). Мутант із
      прибраним відсіканням червонить рівно два нові регрес-тести.
      🔴 Урок процесу: дефект приземлився зеленим, бо жоден тест не
      рендерив картку товару БЕЗ `Product`-розмітки, а фікстура
      `site-singular/product.html` навмисно тримає «схоже» без зображень —
      саме та форма, що маскувала помилку.
      Доки звірено з кодом: `source: 'aria'` ніколи не існувало (aria-label
      агрегується в `anchors` і виходить як `'anchor'`), браузерних файлів
      пʼять, а не чотири.
- [ ] **Ре-валідація проти живого референсу** (Р8): один прогін
      `discover.mjs` проти
      https://deo-ecommerce.nextjsshop-preview.workers.dev/ — очікування
      `listing = /product`, `product = /product/<slug>` без ручних правок.
      Потребує мережі; фікстурний регрес того самого кейсу вже під CLI-смоком
...
- [x] **Виконано 2026-08-18** (трек `claude/theme-views-v3`; фактичні
      ...
      Що приземлилось: view-model-и пʼяти сторінок у субшляху
      `@simplycms/objects/views` (+ `./views/fixtures`) з опційним type-only
      peer на react; `views?` у `ThemeModule` і мʼяка перевірка форми у
      `validateThemeModule`; slot-компоненти 20 комерційних реквізитів із
      маркерами `data-simplycms-requisite`; спліт усіх пʼяти сторінок на
      container + канонічний view (рядків: `ProductDetail` 722→135,
      `Catalog` 748→41, `CatalogSection` 786→85, `Cart` 127→24, `Home` 86→61);
      conformance-kit `@simplycms/themes/conformance` із негативним
      контролем; settings-ланцюг доведено тестами (merge default-ів на обох
      шляхах ініціалізації + інвалідація SSR-кешу з адмінки). Тести
      695 → 803. Жодна тема `views` не постачає (Р11) — це навмисно.
      ~~🔴 Незакритий борг: жива валідація~~ — **закрито 2026-08-19
      прогоном №4** (Opus-агент на стенді, коміт `2647446`): default-тема —
      перший реальний споживач views (`Catalog`/`CatalogSection`/
      `ProductDetail`; `Home`/`Cart` свідомо канонічні), conformance
      зелений ОБОМА каналами (CLI + kit, без моків; негативний контроль
      перевірено живим експериментом — прибраний слот валить гейт із
      точною назвою реквізита), side-by-side проти референсу закрив
      5 структурних рядків таблиці прогону №2 (картки галереї/інфо,
      таби→панелі, trust-бейджі, кластер `transform 500ms`, шкала
      відступів — збіг точний), SSR стенда віддає view теми з
      8 маркерами реквізитів. Гейти 827/827. Артефакти —
      `docs/design-references/ref/{side-by-side-v4,notes-v4}.md` (локально).

      **Кандидати v3.1** (7 знахідок валідації прогону №4, якорі в
      notes-v4.md): `classNames`-мапа слотів замість одного `className`
      (типографіка ціни недосяжна темі); параметризація сітки
      `CatalogProductGrid` (`columns`/`renderItem` — зараз ідентичність
      каталогу замкнена в слоті, `views.Catalog` дає менше важеля, ніж
      `views.ProductDetail`); роутер-стаб у conformance-kit (`Link` у view
      валить гейт, обхід — деградація до `<a>` у темі); окремий канал
      motion/reveal (IntersectionObserver несумісний із jsdom-рендером);
      i18n-глоб `themes/*/views/**` поза eslint-зоною; скан i18n-coverage
      пропускає лише теки `__tests__`; гейт не бачить `display:none`
      (слот «розставлений», але невидимий).

**Трек «теми + клонування дизайну» цим закрито** (рішення власника про
межу — розмова 2026-08-19): механізм клонування end-to-end + контракт v3 +
відшліфована default-тема з власними view-ами. Далі за бажанням власника —
трек B (секційна композиція + drag-and-drop) і v3.1-кандидати вище.
