...
> **[АРХІВ 2026-07-30]** Виконано у 2026-05/06: admin (ssr:false) + _protected + auth роути існують і працюють. Чекбокси історично не відмічались.

> Execution note: ця фаза запускається після завершення storefront SSR routes з Phase 3.
> Це кодова фаза. Не робити повторний audit замість міграції, окрім короткого preflight-а для точного списку admin sub-routes перед створенням route files.
> Якщо Clarify-пункт уже має рекомендований варіант і немає суперечності з Phase 0-3, вважати його прийнятим рішенням для імплементації.
