...
| `.agents/skills/redesign-from-reference/` | Як робити редизайн магазину за референс-сайтом: правові межі, детерміністична інспекція (`scripts/design-import/`), мапінг токенів, тема штатним лайфсайклом, спека-файли компонентів, side-by-side QA. Їде в шаблон скаффолдера (`pnpm template:sync`); посібник — [`docs/guides/redesign-from-reference.md`](docs/guides/redesign-from-reference.md) |
