...
export function ProductModifications({
  productId,
  sectionId,
  ...
  const { data: modPrices } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from('product_prices')
        .select(
          'modification_id, price, old_price, price_types!inner(is_default)',
        )
        .eq('product_id', productId)
  ...
  const getDefaultPrice = (modId: string) => {
    const entry = modPrices?.find(
      (p) => p.modification_id === modId && p.price_types?.is_default,
    );
    return entry;
  };
  ...
  const createMutation = useMutation({
    mutationFn: async (data: Partial<ProductModification>) => {
      // Get max sort_order
      const maxSortOrder =
        modifications?.reduce((max, m) => Math.max(max, m.sort_order), -1) ??
        -1;

      const { error } = await supabase.from('product_modifications').insert([
        {
          ...data,
          product_id: productId,
          sort_order: maxSortOrder + 1,
        } as TablesInsert<'product_modifications'>,
      ]);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      id: string;
      data: Partial<ProductModification>;
    }) => {
  ...
  const openEdit = (modification: ProductModification) => {
    ...
    const existingImages = Array.isArray(modification.images)
      ? (modification.images as string[])
      : [];
          ...
          <Table>
            ...
            <TableBody>
              {modifications.map((mod, index) => {
                const modImages = Array.isArray(mod.images)
                  ? (mod.images as string[])
                  : [];
