...
-- ── 7. Сиквенси ────────────────────────────────────────────────────────────
-- Їх у baseline немає: тип первинного ключа всюди лишається `uuid` — незалежно
-- від того, звідки береться значення (клієнт для Категорії A, `DEFAULT
-- gen_random_uuid()` чи серверний код для Категорії B, Е0). Сиквенс з'явився б
-- лише для `serial`/`identity`-колонки, якої в схемі немає й не буде.
