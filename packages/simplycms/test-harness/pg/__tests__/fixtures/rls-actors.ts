...
export const SEED_STATEMENTS: string[] = [
  ...
  `insert into public.profiles (id, user_id, email) values
     (gen_random_uuid(), '${USER_A}', 'a@example.test'),
     (gen_random_uuid(), '${USER_B}', 'b@example.test')`,
  `insert into public.wishlists (id, user_id, product_id) values
     (gen_random_uuid(), '${USER_A}', '${PRODUCT}'),
     (gen_random_uuid(), '${USER_B}', '${PRODUCT}')`,
  `insert into public.user_addresses (id, user_id, name, city, address) values
     (gen_random_uuid(), '${USER_A}', 'Дім A', 'Київ', 'вул. Перша, 1'),
     (gen_random_uuid(), '${USER_B}', 'Дім B', 'Львів', 'вул. Друга, 2')`,
  ...
  `insert into public.order_items (id, order_id, product_id, name, price, quantity, total) values
     (gen_random_uuid(), '${ORDER_A}', '${PRODUCT}', 'Тестовий товар', 100, 1, 100),
     (gen_random_uuid(), '${ORDER_GUEST}', '${PRODUCT}', 'Тестовий товар', 50, 1, 50)`,
  ...
  `insert into public.product_reviews (id, product_id, user_id, rating, status) values
     (gen_random_uuid(), '${PRODUCT}', '${USER_B}', 5, 'approved'),
     (gen_random_uuid(), '${PRODUCT}', '${USER_A}', 4, 'pending')`,
  ...
  `insert into public.user_roles (id, user_id, role) values
     (gen_random_uuid(), '${USER_A}', 'user'),
     (gen_random_uuid(), '${USER_B}', 'user'),
     (gen_random_uuid(), '${USER_ADMIN}', 'admin')`,
];
...
export const recipientInsert = (userId: string): string =>
  `insert into public.user_recipients
     (id, user_id, first_name, last_name, phone, city, address)
   values (gen_random_uuid(), '${userId}', 'Отримувач', 'Тестовий',
           '+380000000002', 'Київ', 'вул. Третя, 3')`;
...
export const MEDIA_INSERT = `insert into public.media
   (id, entity_type, entity_id, storage_key, size_bytes, mime_type, uploaded_by)
 values (gen_random_uuid(), 'product', '${PRODUCT}', 'products/new.webp', 10,
         'image/webp', '${USER_A}')`;
