...
export const SHOWCASE_FIXTURE_STATEMENTS: string[] = [
  ...
  `insert into public.user_categories (id, name, code, is_default, price_type_id)
   select gen_random_uuid(), 'Гурт', '${WHOLESALE_CODE}', false, id
  ...
  `insert into public.profiles (id, user_id, email, first_name, category_id)
   select gen_random_uuid(), u.id, u.email, 'Роздрібний', c.id
     from public.users u
     cross join public.user_categories c
    where u.email = '${RETAIL_EMAIL}' and c.code = 'retail'`,

  `insert into public.profiles (id, user_id, email, first_name, category_id)
   select gen_random_uuid(), u.id, u.email, 'Гуртовий', c.id
     from public.users u
     cross join public.user_categories c
    where u.email = '${WHOLESALE_EMAIL}' and c.code = '${WHOLESALE_CODE}'`,

  `insert into public.profiles (id, user_id, email, first_name, category_id)
   select gen_random_uuid(), u.id, u.email, '${REVIEWER_FIRST_NAME}', c.id
     from public.users u
     cross join public.user_categories c
  ...
  `insert into public.banners (id, title, image_url, placement, sort_order, is_active)
   values (gen_random_uuid(), '${HIDDEN_BANNER_TITLE}', '/hidden.jpg',
           '${BANNER_PLACEMENT}', 90, false)`,

  `insert into public.banners (id, title, image_url, placement, sort_order, is_active, date_to)
   values (gen_random_uuid(), '${EXPIRED_BANNER_TITLE}', '/expired.jpg',
  ...
  `insert into public.product_reviews (id, product_id, user_id, rating, title, status)
   select gen_random_uuid(), p.id, u.id, ${APPROVED_RATINGS[0]}, 'Чудова панель', 'approved'
     from public.products p cross join public.users u
    where p.slug = '${REVIEWED_PRODUCT_SLUG}' and u.email = '${REVIEWER_EMAIL}'`,

  `insert into public.product_reviews (id, product_id, user_id, rating, title, status)
   select gen_random_uuid(), p.id, u.id, ${APPROVED_RATINGS[1]}, 'Норм', 'approved'
     from public.products p cross join public.users u
    where p.slug = '${REVIEWED_PRODUCT_SLUG}' and u.email = '${RETAIL_EMAIL}'`,

  `insert into public.product_reviews (id, product_id, user_id, rating, title, status)
   select gen_random_uuid(), p.id, u.id, ${PENDING_RATING}, 'Ще на модерації', 'pending'
     from public.products p cross join public.users u
