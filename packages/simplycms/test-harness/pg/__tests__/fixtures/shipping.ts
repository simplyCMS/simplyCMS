...
export const SHIPPING_FIXTURES: string[] = [
  `insert into public.shipping_methods (code, name, is_active, sort_order)
     values ('${ACTIVE_METHOD_CODE}', 'Самовивіз', true, 1),
            ('${HIDDEN_METHOD_CODE}', 'Вимкнений курʼєр', false, 2)`,

  `insert into public.shipping_zones (name, is_active, is_default, cities)
     values ('${ACTIVE_ZONE_NAME}', true, true, '{}'),
            ('${HIDDEN_ZONE_NAME}', false, false, '{}')`,
  ...
  `insert into public.shipping_rates
     (method_id, zone_id, name, calculation_type, base_cost, free_from_amount, is_active)
   select m.id, z.id, 'Основний тариф', 'free_from', ${BASE_COST}, ${FREE_FROM_AMOUNT}, true
     from public.shipping_methods m, public.shipping_zones z
    where m.code = '${ACTIVE_METHOD_CODE}' and z.name = '${ACTIVE_ZONE_NAME}'`,

  `insert into public.shipping_rates
     (method_id, zone_id, name, calculation_type, base_cost, is_active)
   select m.id, z.id, 'Вимкнений тариф', 'flat', 999, false
     from public.shipping_methods m, public.shipping_zones z
    where m.code = '${ACTIVE_METHOD_CODE}' and z.name = '${ACTIVE_ZONE_NAME}'`,

  `insert into public.pickup_points (method_id, name, address, city, is_active)
   select m.id, '${ACTIVE_POINT_NAME}', 'вул. Тестова, 1', 'Київ', true
     from public.shipping_methods m where m.code = '${ACTIVE_METHOD_CODE}'`,

  `insert into public.pickup_points (method_id, name, address, city, is_active)
   select m.id, '${HIDDEN_POINT_NAME}', 'вул. Тестова, 2', 'Київ', false
     from public.shipping_methods m where m.code = '${ACTIVE_METHOD_CODE}'`,
];
