 ...
 * Через це `orders` свідомо присутня в ОБОХ, але по різні боки: сервер
 * генерує їй ключ сам (разом з атомарним `order_number`), тож тут вона під
 * вимогою «передавай id»; DEFAULT у БД при цьому збережено як страхувальна
 * сітка — там вона у списку винятків. Склад Категорії B на рівні DDL —
 * контракт етапу (рішення К3-6), а не дефект.
 */
 ...
 * ключ ніде.
 *
 * 🔴 `orders` тут НЕМАЄ — див. пояснення про дві площини вище: DEFAULT у БД
 * вона зберігає, але сервер усе одно шле ключ явно.
 */
