...
describe('замовлення списує залишок і повертає його при скасуванні', () => {
  ...
  it('🔴 I1-регрес: точку ЄДИНУ й ВИЧЕРПАНУ ДО НУЛЯ деактивували — скасування НЕ фліпає статус в in_stock', async () => {
    ...
    // точці для НОВИХ бронювань не ведеться, `stock-info.ts:77`), і хибний
    ...
    expect(await statusOf(productId)).toBe('out_of_stock');
    // 🔴 Пін і на КІЛЬКІСТЬ, не лише на статус: `reserveStock` виходить
    // першою гілкою «обліку немає» (знімок без `includePointId` порожній),
    // тож рядок деактивованої точки мусить лишитись недоторканим. Без цього
    // асерта майбутня зміна, яка почне списувати з закритої точки, лишила б
    // кейс зеленим.
    expect(await quantities(productId)).toEqual([3]);
  });
