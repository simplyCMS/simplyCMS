 ...
 * нього — грант перевіряється ПЕРШИМ (`requireGrant` усередині `runAdmin`),
 * анонім до розбору взагалі не доходить. Розбір усередині транзакції не
 * тримає зʼєднання на передачі байтів: Start уже буферизував ЦІЛЕ тіло в
 * `FormData` ДО виклику цього хендлера (`@tanstack/start-server-core`,
 * `server-functions-handler`: `await request.formData()`) — `parseUploadForm`
