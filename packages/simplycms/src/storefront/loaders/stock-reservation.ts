...
import { loadTargetStatus, setTargetStatus } from 'simplycms/inventory';
 ...
 * гілку: read-side його теж не бачить (`stock-info.ts:77`).
 *
 * 🔴 М-5: паритет read/write ТУТ НЕ ТОЧНИЙ для НЕактивної СИСТЕМНОЇ точки.
 * `serving` тут і в `servingQuantity` — `is_system OR is_active`
 * (`stock-write.ts:129`), тож склад, знятий з продажу (`is_active = false`),
 * але системний, і далі бере участь у сумі, за якою фліпається статус.
 * `loadStockInfo` (`stock-info.ts:77`) фільтрує рівно `is_active` — жодного
 * винятку для `is_system` — тож той самий склад НІКОЛИ не потрапляє ні в
 * `byPoint`, ні в `totalQuantity`. Асиметрія навмисна (read-side — що
 * ПРОПОНУВАТИ покупцю, write-side — що ОБЛІКОВУВАТИ), але це ДВІ різні
 * множини точок, а не одна й та сама, — паритетом їх називати не можна.
 *
