...
import type { ShippingRate, ShippingCalculationContext } from '../shipping';
...
function makeCtx(
  over?: Partial<ShippingCalculationContext>,
...
describe('calculateShippingCost', () => {
  ...
  it('free_from returns 0 above threshold', () => {
    const rate = makeRate({
      ...
      free_from_amount: 500,
    });
    expect(
      calculateShippingCost(
        rate,
        makeCtx({ cart: { items: [], subtotal: 600 } }),
      ),
    ).toBe(0);
  });
  ...
  it('order_total returns percentage', () => {
    ...
    expect(
      calculateShippingCost(
        rate,
        makeCtx({ cart: { items: [], subtotal: 1000 } }),
      ),
    ).toBe(100);
  });
