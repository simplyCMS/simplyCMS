...
// Перенесено з core/lib/discountEngine; типи — з simplycms/contracts.
...
function evaluateGroup(
  ...
  if (!isWithinDateRange(group.starts_at, group.ends_at, now))
    return { totalAmount: 0, applied, rejected };

  // Evaluate all direct discounts in this group (sorted by priority)
  const sortedDiscounts = [...group.discounts].sort(
    (a, b) => a.priority - b.priority,
  );
  const evalResults: EvalResult[] = sortedDiscounts.map((d) =>
    evaluateDiscount(d, basePrice, ctx, now),
  );
  ...
  const childResults: GroupResult[] = sortedChildren.map((child) =>
    evaluateGroup(child, basePrice, ctx, now),
  );
