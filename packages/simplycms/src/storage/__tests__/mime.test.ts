...
describe('sniffImageMime', () => {
  ...
  it('JPEG', () => {
    expect(
      sniffImageMime(bytes(0xff, 0xd8, 0xff, 0xe0, 0, 0, 0, 0, 0, 0, 0, 0)),
    ).toBe('image/jpeg');
  });

  it('GIF87a і GIF89a', () => {
    expect(
      sniffImageMime(concat(ascii('GIF87a'), bytes(0, 0, 0, 0, 0, 0))),
    ).toBe('image/gif');
    expect(
      sniffImageMime(concat(ascii('GIF89a'), bytes(0, 0, 0, 0, 0, 0))),
    ).toBe('image/gif');
  });
  ...
  it('WebP — RIFF ... WEBP', () => {
    expect(
      sniffImageMime(
        concat(ascii('RIFF'), bytes(1, 2, 3, 4), ascii('WEBP'), bytes(0, 0)),
      ),
    ).toBe('image/webp');
  });
  ...
  it('AVIF — ftyp із брендом avif/avis', () => {
    expect(
      sniffImageMime(
        concat(bytes(0, 0, 0, 0x20), ascii('ftyp'), ascii('avif'), bytes(0, 0)),
      ),
    ).toBe('image/avif');
    expect(
      sniffImageMime(
        concat(bytes(0, 0, 0, 0x20), ascii('ftyp'), ascii('avis'), bytes(0, 0)),
      ),
    ).toBe('image/avif');
  });
  ...
  it('SVG відбивається, і з XML-прологом теж', () => {
    expect(
      sniffImageMime(ascii('<svg xmlns="http://www.w3.org/2000/svg"/>')),
    ).toBeNull();
  ...
  it('RIFF без WEBP (наприклад wav) відбивається', () => {
    expect(
      sniffImageMime(
        concat(ascii('RIFF'), bytes(1, 2, 3, 4), ascii('WAVE'), bytes(0, 0)),
      ),
    ).toBeNull();
  });
