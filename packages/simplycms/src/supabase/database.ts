...
//
// Baseline типів CORE-схеми SimplyCMS: снапшот, знятий генератором Supabase
// CLI на еталонній dev-БД БЕЗ плагінів.
//
// 🔴 ЗАМОРОЖЕНИЙ (0.4.1). Генератора більше немає: `pnpm db:generate-types`,
// `pnpm types:baseline` і host-генерат `supabase/types.ts` знесені разом із
// Supabase-контуром магазину. Файл лишається рівно доти, доки на ньому
// типізується адмінка, — її переписує трек К3; після нього файл іде за
// генератором. Джерело типів для НОВОГО коду — `simplycms/schema/types`
// (виведені з Drizzle-схеми, рішення B12).
