...
| `simplycms/contracts/server-only` | Декларація межі довіри клієнт/сервер. Дані: `SERVER_ONLY` (субшляхи ядра, що існують лише на сервері) і `SERVER_ONLY_DEPS` (обʼєкти `{ name, clientSafe? }` — `clientSafe` перелічує підшляхи, дозволені клієнту, як `better-auth/react`). Хелпери: `serverOnlyOwner`/`isServerOnlySubpath` (класифікація субшляху), `serverOnlyDepSpecifier` (патерн однієї залежності з урахуванням `clientSafe`) і `importProtection()` — повний обʼєкт опції Import Protection Start (читач 6), який три `vite.config.ts` передають одним рядком; єдине зовнішнє ребро T0 — `import type` з peer-пакета `@tanstack/react-start`. Лише дані — читачів ШІСТЬ: збірка ядра, гейт `dist-server-boundary`, правило `server-only-relative`, групи `no-restricted-imports` плагінів, Gate C, Import Protection у `vite.config.ts` магазину (таблиця з негативними контролями — `docs/architecture/test-contours.md` §12) |
