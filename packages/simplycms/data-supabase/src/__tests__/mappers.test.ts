...
describe("mapProduct", () => {
  it("maps a full product row to the domain shape", () => {
    const row = {
      ...
      product_modifications: [
        { id: "m1", sku: "SKU1", stock_status: "in_stock", is_default: true, sort_order: 0 },
      ],
      product_prices: [
        { price_type_id: "retail", price: "100", old_price: "120", modification_id: null },
      ],
      images: ["/a.jpg"],
    };
  ...
  it("normalizes invalid stock_status to null and defaults arrays", () => {
    const p = mapProduct({ id: "p2", slug: "x", name: "X", stock_status: "weird" });
...
describe("mapSection", () => {
  it("maps a section row", () => {
    const s = mapSection({ id: "s1", slug: "cat", name: "Cat", parent_id: null, sort_order: 2 });
    expect(s).toMatchObject({ id: "s1", slug: "cat", name: "Cat", sort_order: 2, is_active: true });
  });
});
...
describe("mapOrder", () => {
  it("maps an order row with joined status and items", () => {
    const o = mapOrder({
      ...
      delivery_city: "Kyiv",
      order_items: [
        { product_id: "p1", modification_id: null, name: "Widget", quantity: 2, price: 50 },
      ],
...
describe("buildDiscountTree", () => {
  it("nests child groups under parents and attaches discounts", () => {
    ...
    const groups = [
      { id: "root", name: "Root", operator: "and", is_active: true, priority: 0, parent_group_id: null },
      { id: "child", name: "Child", operator: "or", is_active: true, priority: 0, parent_group_id: "root" },
    ];
