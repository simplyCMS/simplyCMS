 ...
 *   нічого.
 *
 * Решта сторінок (Catalog/CatalogSection/ProductDetail) лишає повний набір:
 * там крайній стан міняє ВМІСТ слота, а не забирає сам слот із лейауту.
 * Точність заради — один виняток у проді: `ProductModificationPicker` віддає
 * `null` для ≤1 модифікації, тож на товарі без варіантів маркера
 * `product-detail.modification-selector` у DOM не буде. Гейт це не хитає й
 * не покриває: kit рендерить не справжні слоти, а стаби, які маркер малюють
 * завжди — він доводить лише те, що view слот РОЗСТАВИВ.
 *
