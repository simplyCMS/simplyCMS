...
| `storefront/` | `simplycms/storefront` | T2 | SSR-лоадери (`withStorefrontDb` над Drizzle) + SEO-генератори — без Supabase. Upward-виняток `db`+`auth`+`storage`+`inventory`: `withActor`, читання сесії, канал до файлів (аватар) і спільний гвард залишків — той самий набір, що в `admin-server` |
| `admin-server/` | `simplycms/admin-server` (+ службовий `./admin-server/impl`) | T2 | Серверний шар адмінки (трек К3, Е1б): фабрика операцій `defineAdminResource` + іменовані операції з інваріантами; публічна поверхня `index.ts` — ЛИШЕ топ-рівневі `createServerFn` (К3-4′), нутрощі — за bare-субшляхом `impl` (дзеркало `storefront/loaders` для Gate C, К3-9′). Upward-виняток `db`+`auth`+`storage`+`inventory`, як у `storefront`: `withActor`, `requireGrant`, канал до файлів і спільний гвард залишків |
| `storage/` | `simplycms/storage` | T2 | 🔴 Порт сховища файлів (К3-Е2): драйвер `local-fs` (іммутабельні ключі `ab/<uuid>.<ext>`, атомарна публікація через `link`), `inspectUpload` (MIME за магічними байтами, SVG заборонений), `writeMedia`/`eraseMedia` — файл і рядок `media` в ОДНІЙ транзакції актора, `serveMedia` для роуту `/media/$`. **server-only** за `contracts/server-only`; upward-виняток `db` — канал до Postgres лише `withActor`. Клієнту звідси не потрібно нічого: резолв URL живе в `domain/media` (T1) |
| `inventory/` | `simplycms/inventory` | T2 | 🔴 Облік залишків (Е3-5): спільний гвард `stock_status` — `loadTargetStatus`/`setTargetStatus`/`syncStatusWithQuantity`, пише в `products`/`product_modifications` через ActorDb. Одна копія на вітрину (`storefront/loaders/stock-*`) і адмінку. **server-only** за `contracts/server-only`; upward-виняток лише `db` — канал до Postgres той самий `withActor`. Не плутати з `simplycms/domain/inventory` (T1, чиста арифметика без IO, клієнтобезпечна) |
...
4. **`files`, `license: MIT`, `README.md`** — конвенція; тестами стережуться
   ...
   ні, не забудь руками.
5. **Збірка** — tsdown. У флагмані `tsdown.config.ts` — ДВІ збірки
   (клієнтська й серверна), а entry виводяться з dev-`exports`
   (ключ → джерело), тож `dist/` дзеркалить `publishConfig.exports`
   за побудовою; server-only групу задає `contracts/server-only`.
   `entry` — МАПА `<шлях у dist> → файл`: за списковою формою бандлер бере
   outbase = спільний предок і мовчки затирає корінь пакета.
   🔴 **`target: 'esnext'` живе у спільному `base`.** Без нього бандлер
   лоуерить `import.meta` у `var import_meta = {}`, і опублікований `dist`
   читає `({}).env` — браузерний Supabase-клієнт падає на гідрації (спіймано
   К0: при злитті 21 конфігу опція вціліла лише в одному профілі). Гард —
   `tests/dist-import-meta.test.ts` у packaging-suite. Розкладка entry — не
   політика синтаксису.
