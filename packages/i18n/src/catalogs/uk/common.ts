...
export const messages = {
  ...
  // Адреси й отримувачі. Сюди — бо ці самі рядки рендеряться у ДВОХ доменах:
  // CRUD-віджети адрес/отримувачів існують паралельно в чекауті
  // (`@simplycms/checkout-ui`) і в профілі (`@simplycms/profile-ui`).
  //
  // 🔴 Префікс «м.» — параметризований, а не окремий рядок поруч зі значенням:
  // англійська міста так не позначає, тож при хардкоді «м.» англомовний
  // магазин показував би «м. Kyiv». Ключ несе ВЕСЬ рядок разом із порядком слів.
  'common.cityShort': 'м. {city}',
  'common.cityAddress': 'м. {city}, {address}',
  'common.address.new': 'Нова адреса',
  'common.address.added': 'Адресу додано',
  'common.address.updated': 'Адресу оновлено',
  'common.recipient.new': 'Новий отримувач',
  'common.recipient.added': 'Отримувача додано',
  'common.recipient.updated': 'Отримувача оновлено',

  // Вартість доставки. У `common`, бо трапляється у ДВОХ доменах — чекаут
  // (`checkout-ui`) і адмінка тарифів (`admin/ShippingZoneEdit`).
  //
  // 🔴 Ці два рядки раніше жили ВСЕРЕДИНІ `@simplycms/domain/shipping` —
  // чистого T1-пакета, який за тірами не має права знати про i18n. Через це
  // англомовний магазин показував у підсумку замовлення «За тарифами».
  // Тепер `formatShippingCost` приймає їх параметром: домен володіє логікою
  // (нуль → безкоштовно, `null`/відʼємне → за тарифами), а текст лишається
  // за кол-сайтом.
  'common.shipping.byTariff': 'За тарифами',
  'common.shipping.free': 'Безкоштовно',
} as const;
