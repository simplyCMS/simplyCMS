...
const eslintConfig = [
  ...
  },
  {
    ignores: [
      ...
      //
      // 🔴 Відтоді під негацію потрапила й СИНКОВАНА копія —
      // `template/scripts/design-import/**` (тулінг редизайну за референсом,
      // `SYNCED_DIRS`). Тобто ті самі файли лінтуються двічі: у монорепо й у
      // шаблоні. Прийнято свідомо: копія байт-ідентична (стереже
      // `tests/create-store-template-parity.test.ts`), у design-import 0
      // ворнінгів, тож норма «0 errors / 13 warnings» від подвійного проходу
      // не пливе, а звужувати негацію означало б ховати виконуваний код
      // шаблону від гейта заради косметики.
      //
